...
static void spmap_it__many(void **state) {
	...
	const struct SImapt *it = spmap_it(map);
...
static void spmap_match_it__many(void **state) {
	...
	const struct SImapt *it = spmap_match_it(map, mock_3pred_str_ptr, D0);
	assert_non_nul(it);
...
static void spmap_match_key_it__many(void **state) {
	...
	const struct SImapt *it = spmap_match_key_it(map, mock_2pred_str, D0);
	assert_non_nul(it);
	assert_str_equal(it->key, "1");
	assert_ptr_equal(it->val, V1);
...
static void spmap_match_val_it__many(void **state) {
	...
	const struct SImapt *it = spmap_match_val_it(map, mock_2pred, D0);
	assert_non_nul(it);
	assert_str_equal(it->key, "1");
	assert_ptr_equal(it->val, V1);
...
static void spmap_remove_all__(void **state) {
	...
	assert_int_equal(spmap_remove_all(map, from), 1);
...
static void spmap_remove_all_free__(void **state) {
	...
	assert_int_equal(spmap_remove_all_free(map, from), 1);
...
static void spmap__null_inputs(void **state) {
	...
	assert_int_equal(spmap_remove_all(NULL, NULL), 0);
	assert_int_equal(spmap_remove_all(map, NULL), 0);
	assert_int_equal(spmap_remove_all(NULL, map), 0);
